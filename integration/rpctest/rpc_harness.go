...
// Harness fully encapsulates an active jind process to provide a unified
// platform for creating rpc driven integration tests involving jind. The
// active jind node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active jind process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*jinutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the jind node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []jinutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure jind properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created jind
...
func (h *Harness) NewAddress() (jinutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() jinutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate jinutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate jinutil.Amount) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*jinutil.Tx, blockVersion int32,
	blockTime time.Time) (*jinutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*jinutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*jinutil.Block, error) {
	...
	prevBlock := jinutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "jind", "rpctest")
