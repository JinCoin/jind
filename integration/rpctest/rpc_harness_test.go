...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt jinutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(jinutil.Amount(5 * jinutil.SatoshiPerBitcoin))
	...
	txid = genSpend(jinutil.Amount(500 * jinutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(jinutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*jinutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, jinutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := jinutil.Amount(250 * jinutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = jinutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := jinutil.Amount(50 * jinutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := jinutil.Amount(numMatureOutputs * 50 * jinutil.SatoshiPerBitcoin)
