...
type memWallet struct {
	...
	addrs map[uint32]jinutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	// newly generated jincoin we can spend.
	addrs := make(map[uint32]jinutil.Address)
...
// SetRPCClient saves the passed rpc connection to jind as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*jinutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          jinutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (jinutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []jinutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
// fundTx attempts to fund a transaction sending amt jincoin. The coins are
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt jinutil.Amount, feeRate jinutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := jinutil.Amount(txSize * int(feeRate))
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate jinutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt jinutil.Amount
	for _, output := range outputs {
		outputAmt += jinutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() jinutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance jinutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (jinutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := jinutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
