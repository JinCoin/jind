...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *jinutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *jinutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how jind may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount jinutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnBtcdConnected
	case btcjson.jindconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		connected, err := parsejindconnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid jind connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a jind extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*jinutil.Tx, error) {
	...
	transactions := make([]*jinutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = jinutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*jinutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return jinutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := jinutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parsejindconnectedNtfnParams parses out the connection status of jind
// and jinwallet from the parameters of a jindconnected notification.
func parsejindconnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance jinutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := jinutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a jind extension and requires a websocket connection.
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []jinutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []jinutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []jinutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a jind extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []jinutil.Address,
...
// NOTE: This is a jind extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []jinutil.Address, outPoints []wire.OutPoint) error {
