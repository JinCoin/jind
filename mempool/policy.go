...
const (
	...
	DefaultMinRelayTxFee = jinutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee jinutil.Amount) int64 {
	...
	if minFee < 0 || minFee > jinutil.MaxSatoshi {
		minFee = jinutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *jinutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee jinutil.Amount) bool {
...
func checkTransactionStandard(tx *jinutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee jinutil.Amount,
...
func GetTxVirtualSize(tx *jinutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
