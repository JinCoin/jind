...
jind provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, jind supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Jincoin
...
Configuring jind as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the jind command
line or in the jind configuration file.  Typically the Tor proxy address will be
...
By default, jind assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since jind will act as the server.
...
```text
HiddenServiceDir /var/tor/jind
HiddenServicePort 23099 127.0.0.1:23099
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring jind as a Tor hidden service requires
three flags:
...
```bash
$ ./jind --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
jind provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the jind command line or in the jind
...
```bash
$ ./jind --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
jind provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./jind --proxy=127.0.0.1:9050 --torisolation
```
