...
const (
	defaultConfigFilename        = "jind.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "jind.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-jind.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = jinutil.AppDataDir("jind", false)
...
// config defines the configuration options for jind.
//
...
type config struct {
	...
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 23099, testnet: 33099)"`
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 23100, testnet: 33100)"`
	RPCCert              string        `long:"rpccert" description:"File containing the certificate file"`
	RPCKey               string        `long:"rpckey" description:"File containing the certificate key"`
	RPCMaxClients        int           `long:"rpcmaxclients" description:"Max number of RPC clients for standard connections"`
	RPCMaxWebsockets     int           `long:"rpcmaxwebsockets" description:"Max number of RPC websocket connections"`
	RPCMaxConcurrentReqs int           `long:"rpcmaxconcurrentreqs" description:"Max number of concurrent RPC requests that may be processed concurrently"`
	RPCQuirks            bool          `long:"rpcquirks" description:"Mirror some JSON-RPC quirks of Jincoin Core -- NOTE: Discouraged unless interoperability issues need to be worked around"`
	...
	Generate             bool          `long:"generate" description:"Generate (mine) jincoins using the CPU"`
...
//
// The above results in jind functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		jindLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = jinutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]jinutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := jinutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		jindLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-jind.conf to the given destination path,
...
// jindDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func jindDial(addr net.Addr) (net.Conn, error) {
...
// jindLookup resolves the IP of the given host using the correct DNS lookup
...
func jindLookup(host string) ([]net.IP, error) {
