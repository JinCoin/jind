...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-jind.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "jind")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of jind binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-jind.conf")
