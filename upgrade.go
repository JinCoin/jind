...
// oldjindHomeDir returns the OS specific home directory jind used prior to
// version 0.3.3.  This has since been replaced with jinutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldjindHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "jind")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".jind")
	}
...
// location prior to jind version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to jind
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldjindHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "jind.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "jind_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "jind_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to jind
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		jindLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old jind.conf into new location if needed.
		...
		if ohpEmpty {
			...
			jindLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to jind as new versions require it.
