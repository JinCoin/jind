...
type Config struct {
	...
	ProcessBlock func(*jinutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *jinutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), jinutil.Amount(coinbaseTx.Value))
	return true
}
