...
const (
	...
	MinHighPriority = jinutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr jinutil.Address) (*jinutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *jinutil.Tx, height int32) error {
...
func logSkippedDeps(tx *jinutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress jinutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*jinutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := jinutil.NewTx(coinbaseTx.MsgTx().Copy())
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A jinutil.Block should use saved in the state to avoid
