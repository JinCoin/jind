...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: jinutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := jinutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*jinutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, jinutil.NewTx(tx))
	}
...
func additionalCoinbase(amount jinutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee jinutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee jinutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee jinutil.Amount) *wire.MsgTx {
