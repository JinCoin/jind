...
// BlockChain provides functions for working with the jincoin block chain.
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *jinutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *jinutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *jinutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *jinutil.Block) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *jinutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *jinutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *jinutil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*jinutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]spentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*jinutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *jinutil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *jinutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *jinutil.Block, *UtxoViewpoint) error
}
