...
const (
	...
	baseSubsidy = 192.96 * jinutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *jinutil.Tx) bool {
...
func IsFinalizedTransaction(tx *jinutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *jinutil.Tx) error {
	...
	// as a satoshi.  One jincoin is a quantity of satoshi as defined by the
	...
	for _, txOut := range msgTx.TxOut {
		...
		// is detected and reported.  This is impossible for Jincoin, but
		...
		if totalSatoshi > jinutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *jinutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *jinutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *jinutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *jinutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *jinutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *jinutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *jinutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *jinutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *jinutil.Block, view *UtxoViewpoint) error {
...
// the jincoins and therefore allowed to spend them.  As it checks the inputs,
// it also calculates the total fees for the transaction and returns that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(tx *jinutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// jincoin is a quantity of satoshi as defined by the
		// SatoshiPerBitcoin constant.
		originTxSatoshi := utxoEntry.AmountByIndex(originTxIndex)
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", jinutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > jinutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				jinutil.Amount(originTxSatoshi),
				jinutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > jinutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
	...
	// NOTE: jincoind checks if the transaction fees are < 0 here, but that
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *jinutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *jinutil.Block) error {
	b.chainLock.Lock()
