...
func addrToKey(addr jinutil.Address) ([addrKeySize]byte, error) {
	...
	case *jinutil.AddressWitnessScriptHash:
		...
		// push to 20-bytes.
		copy(result[1:], jinutil.Hash160(addr.ScriptAddress()))
		return result, nil
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*jinutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *jinutil.Block, view *blockchain.UtxoViewpoint) {
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *jinutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *jinutil.Block, view *blockchain.UtxoViewpoint) error {
	// Build all of the address to transaction mappings in a local map.
	addrsToTxns := make(writeIndexData)
	idx.indexBlock(addrsToTxns, block, view)
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr jinutil.Address, numToSkip, numRequested uint32, reverse bool) ([]database.BlockRegion, uint32, error) {
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil, 0, err
	}
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *jinutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*jinutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *jinutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr jinutil.Address) []*jinutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*jinutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*jinutil.Tx),
