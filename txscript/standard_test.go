...
// newAddressPubKey returns a new jinutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) jinutil.Address {
	addr, err := jinutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new jinutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) jinutil.Address {
	addr, err := jinutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new jinutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) jinutil.Address {
	addr, err := jinutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the jinutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the jinutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := jinutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := jinutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := jinutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := jinutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := jinutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*jinutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*jinutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*jinutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*jinutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*jinutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
...
var scriptClassTests = []struct {
	...
	// codeseparator parts have been elided. (jincoin core's checks for
